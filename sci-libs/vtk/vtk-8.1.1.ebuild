...
SLOT="0"
IUSE="
	all-modules aqua boost doc egl examples imaging ffmpeg gdal java json kaapi mpi
...
# Qt5 gles2 use flag disables desktop opengl, which we need.
#	Look into this later: dev-libs/libexecinfo
RDEPEND="
	app-arch/lz4
	dev-libs/expat
	dev-libs/jsoncpp:=
	...
	qt5? (
		...
		dev-qt/qtsql:5
		dev-qt/qtwebkit:5[-gles2]
		dev-qt/qtx11extras:5
		python? ( dev-python/PyQt5[-gles2,${PYTHON_USEDEP}] )
	)
	R? ( dev-lang/R )
	tbb? ( dev-cpp/tbb )
	tcl? ( dev-lang/tcl:0= )
...
# At the moment, this conflicts with xdmf3 installed by vtk if we have it installed: xdmf2? ( sci-libs/xdmf2 )"
...
src_prepare() {
	local x
	# Need to setup to use system packages for: mpi4py, websockets (wslink), xmdf3 (xdmf2 needs slotting, headers to subdir) 
	...
	# The following don't have mangled symbols or have conflicting install directories
	for x in constantly hyperlink incremental Twisted txaio ZopeInterface; do
	...
	#Fix FUBAR install paths in bundled thirdparty libs:
	sed -e 's:${CMAKE_INSTALL_PREFIX}/lib:${VTK_INSTALL_LIBRARY_DIR}:g' \
		-e 's:DESTINATION lib:&${LIB_SUFFIX}:g' \
		-i $(find ThirdParty -name CMakeLists.txt) || die
	sed -e '/set(VTKm_INSTALL_LIB_DIR "lib")/ s/lib/${VTK_INSTALL_LIBRARY_DIR}/' \
		-i ThirdParty/vtkm/vtk-m/CMakeLists.txt || die
...
src_configure() {
	...
	local mycmakeargs=(
		...
		# No, we REALLY don't want rpaths on our installed files.
		-DCMAKE_SKIP_RPATH=YES
		-DCMAKE_BUILD_WITH_INSTALL_RPATH=OFF
		-DCMAKE_INSTALL_RPATH_USE_LINKPATH=FALSE
		-DHAVE_RPATH=FALSE
		-DXMDF_SET_RPATH=OFF
		...
		-DVTK_BUILD_ALL_MODULES_FOR_TESTS=$(usex test $(usex all-modules) OFF)
		-DVTKm_ENABLE_TESTING=$(usex test)
		-DVERDICT_ENABLE_TESTING=$(usex test)

		# Path and libname configuration
		-DVTK_DIR="${S}"
		...
		# The following don't have mangled symbols or have conflicting install directories
		# six autobahn constantly hyperlink incremental Twisted txaio ZopeInterface
		...
		# The following have symbols mangled for vtk, let's use them until we have a curated kit of them tested to work with vtk & friends.
		...
		# FIXME - This doesn't have mangled symbols, but we don't have a package for it either.
		...
		# -DVTK_WRAP_TCL=$(usex tcl) # Deprecated in vtk-8.1
	...
	if use qt5; then
		mycmakeargs+=(
			...
			-DQT_WRAP_CPP=ON
			-DQT_WRAP_UI=ON
			-DVTK_INSTALL_QT_DIR="qt5/plugins/designer"
			-DVTK_INSTALL_QT_PLUGIN_DIR="$(qt5_get_libdir)/qt5/plugins/designer"
